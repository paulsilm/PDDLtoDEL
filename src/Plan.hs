...
findShortestICPlan pddl True d debug
  ...
  where maxDepth = if d == 0 then maxBound :: Int else d
        planList = [concatMap ppICPlan plan 
                   | plan <- [dls i debug $ pddlToDEL pddl | i <- [1..maxDepth]] -- Iterative DFS
                   , plan /= []] -- :: [[String]]
...
dls :: (Typeable action, Eq state, Update state action) => Int -> Bool -> CoopTask state action -> [ICPlan action]
dls d debug (CoopTask now acts goal)
